...
<?php
if($category=='grocery'){
	include str_replace('\\','/',$_SERVER['DOCUMENT_ROOT']).'/services/grocery/grocery_products_header.php';
	$_SESSION['grocery_setup'] = true;
}else{
	$_SESSION['grocery_setup'] = null;
}
?>
...
while($row){
	...
	echo ($c_p%4==0 || $c_p==0?'<div class=row >':'').'<div class=col-sm-3 >'.(ISSET($_GET['s'])?(($_GET['s']*40)==$c_p?'<a name=current >':''):'').'<div class=col-sm-3 >';
...
' transaction t WHERE t.product_id='.$row['id'];
	...
	$row3 = $result3->fetch_assoc();
          ...
          echo 
		   '</div><div>'.
		   '<div class="force_right" ><div class="variance_p" >';
	...
	'</table>';
		   ...
		   '</tr>'.
		   '</table>'.
		   '<a href="open.php?product_id='.$row['id'].'" ><img '.($main?'id=main_product_image':'').' class=main_product_image src="'.(strpos($row2["url"],'//')!==false?$row2["url"]:'product_images/'.$row2["url"]).'" /></a>'.
		   
		   ($main && ISSET($more_than_one_product)?
		...
		$result2 = $conn->query("SELECT logo,name,website FROM company WHERE id=".$row["company_id"]);
           ...
           $row2 = $result2->fetch_assoc();
		   echo 
		   (!$in_product && $category=='grocery'?'<div class=add_to_cart ><input id=cart_qty_'.$row['id'].' type=number value=1 placeholder="Qty" /><a class="btn btn-primary" id=add_to_cart href=# onclick="addToCart(\'shopping_cart.php?product_id='.$row['id'].'&params=&qty=\'+e(\'cart_qty_'.$row['id'].'\').value,this);return false;" >add to cart</a></div>':'').
           '<table class=p_head >'.
		   '<tr>'.
		   '<td>'.
		   ...
		   '<td>'.
		   '<span class=date ><a class=fshare href="https://facebook.com/sharer.php?u=https://www.productlists.co.za/services/sell/products/products.php?product_id=
'.$row['id'].'" onclick="javascript:window.open(this.href, \'\', \'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600,top=150, left=\'+((screen.width/2)-300));return false;" >Share</a></span>'.
	       ...
	       '<td><a class="feeling love" href=# onclick=\'feelingResponse(sendreq("services/sell/products/feature/feeling/love.php"),this);return false;\'  >'.($row['love']?$row['love']:'').'</a></td>'.
		   '<td><a class="feeling angry" href=# onclick=\'feelingResponse(sendreq("services/sell/products/feature/feeling/angry.php"),this);return false;\' >'.($row['angry']?$row['angry']:'').'</a></td>'.
		   '<td><a class="feeling happy" href=# onclick=\'feelingResponse(sendreq("services/sell/products/feature/feeling/happy.php"),this);return false;\' >'.($row['happy']?$row['happy']:'').'</a></td>'.
		   '</table>'.
		   '</div>';
		   
		   echo
		   '</div></div></div>'.($c_p%4==0 || $c_p==0?'</div>':''); 
	...
	(!$has_products?'Requested <strong>product</strong> is not found.<br><br>':'');
